...
def _model_train(df,tag,test=False):
    ...
    if not test:
        ...
        data_file = os.path.join(MODEL_DIR, '{}-latest-train.pickle'.format(tag))
...
def model_train(data_dir=None,test=False, clean=False):
    """
    ...
    ts_data = fetch_ts(data_dir, clean=clean)
...
def model_load(prefix='sl',data_dir=None,training=True, clean=False):
    """
    ...
    models = [f for f in os.listdir(os.path.join(".","models")) if re.search(prefix,f) and re.search('joblib',f)]
...
def model_predict(country,year,month,day,all_models=None,test=False):
    ...
    for i in range(2):
        ...
        if not all_models:
            all_data,all_models = model_load(training=False, clean=clean)
        ...
        if target_date not in data['dates']:
            if i == 1:
                ...
                print("WARNING (model_predict) - date {} not in range {}-{}, trying to reload invoices data".format(target_date,
                                                                                            data['dates'][0],
                                                                                            data['dates'][-1]))
                clean=True
                all_models=None
